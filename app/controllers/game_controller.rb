class GameController < ApplicationController
  ...
  caches_page :splash
  # NOTE: static nowadays since you can't login
  caches_page :results
  ...
  def answer
    ...
    @current_user.attempts.create({
      :target_facebook_id => result[0]["uid"],
  ...
  def results
    ...
    @overall_correct_pct = Rails.cache.fetch("overall", :expires_in => 7.days) do
	  ...
	  @gender_age_breakdown = Rails.cache.fetch("gender_age", :expires_in => 7.days) do
	  	#recache
    ...
    @gender_gender_breakdown = Rails.cache.fetch("gender_gender", :expires_in => 7.days) do
      Attempt.connection.select_all("
